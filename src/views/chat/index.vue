        ...
        <div v-if="user" style="width:1100px;height: 700px ;margin: auto auto; background-color:rgb(250,250,250) ">
            <div class="chat-user" style="width:20%;height: 100% ;float: left">
                <div class="chat-user-self" style="backgroundColor:rgb(255,72,67);height: 15%;width: 100%">
                    ...
                    <div>
                        <h3>
                            用户列表 <span style="color:#108ee9">{{userList.length}}</span>
                        </h3>
                    </div>
                </div>
  ...
  export default {
    ...
    methods: {
      ...
      pointToPointChatInit(item) {
        ...
        //向后端发送请求 将消息置为已读
        this.$http.post(this.url + '/chat/stateMessage', JSON.stringify(this.oneToOneMessage)).then(res => {
        }).catch(e => {
          console.log(e)
        })
      ...
      initWebSocket() {
        ...
        stompClient.connect({}, () => {
          ...
          stompClient.subscribe('/chat/point/' + this.user, (msg) => {
            const chatMessage = JSON.parse(msg.body)
            this.addUserList(chatMessage.sender)
          ...
          stompClient.send('/app/chat.addUser',
            ...
            // 向/app/chat.userUp发送消息将该用户的名称告知服务器,获取私聊消息
            stompClient.send('/app/chat.userUp',
              {},
              JSON.stringify({ sender: this.user, type: 'JOIN', icon: this.$store.getters.userIcon })
            )
    ...
    watch: {
      ...
      'userList': function(newVal, oldVal) {
        console.log(newVal)
      }
    },
