        ...
        <div v-if="user" style="width:1100px;height: 700px ;margin: auto auto; background-color:rgb(250,250,250) ">
            <div class="chat-user" style="width:20%;height: 100% ;float: left">
                ...
                <div class="chat-user-info" style=" overflow: auto;overflow-y: scroll;height: 85%">
                    ...
                    <div style=" padding:10px; text-align: center;width: 100% ;height: 40px;" class="userElement"
                         @click="manyChatInit()" id="manyChatInit">多人聊天
                    </div>
                    <div v-for="(item ,index) in userList" class="userElement"
                         style="padding:5px;margin-bottom: 5px; text-align: center;width: 100% ;height: 30px;"
                         :id="item"
                         v-if="user!==item" @click="pointToPointChatInit(item)">
                        {{ item }}
                    </div>
                </div>
            </div>
            <div class="chat-message" style="float: left;height: 100%; width:80%;" v-if="manyChat">
                ...
                <div class="chat-message-send" style="height: 20%">
                    <div style="float: left;width:80%;margin:0px 10px 0px 10px ">
                        <textarea v-model="content" @keyup.enter="sendMessage" type="textarea" class="send-control"
                                  placeholder="Type a message..."></textarea>
                    </div>
                    <div style="float: left;width: 10%;margin: 5px 0px 5px 10px">
                        <a-button type="primary" @click="sendMessage" style="width: 100px">
                            发送
                        </a-button>
                    </div>
                    <!--                       聊天记录-->
                    <div style="float: left;width: 10%; margin: 5px 0px 5px 10px">
                        <a-button type="primary" @click="clickShowModal" style="width: 100px">
                            聊天记录
                        </a-button>
                    </div>
                    <a-modal v-model="visible" title="聊天记录" @ok="handleOk">
                        <ul style=" list-style-type: none;margin: 0; overflow: auto;overflow-y: scroll;padding: 0 20px 0px 20px;height: 600px;">
                            <ChatItem v-for="(message ,index) in messageRecordList" :key="index"
                                      :message="message"/>
                        </ul>
                    </a-modal>
                </div>
            </div>
            <div class="chat-message" style="float: left;height: 100%; width:80%;" v-if="pointChat">
                <div class="chat-message-top" style="height: 10%">
                    <div style=" padding:10px; text-align: center;width: 100%;">
                        <h2>{{pointUser}}</h2>
                        <hr>
                ...
                <div class="chat-message-send">
                    <div style="float: left;width:80%;margin:0px 10px 0px 10px ">
                        <textarea v-model="content" @keyup.enter="sendPointMessage(pointUser)" type="textarea"
                                  class="send-control"
                                  placeholder="Type a message..."></textarea>
                    </div>
                    <div style="float: left;width: 10%;margin: 5px 0px 5px 10px">
                        <a-button type="primary" @click="sendPointMessage(pointUser)" style="width: 100px">
                            发送
                        </a-button>
                    </div>
                </div>
